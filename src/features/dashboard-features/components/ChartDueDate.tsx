...
const ChartDueDate: React.FC<ChartDueDateProps> = ({ year, month }) => {
    ...
    useEffect(() => {
        if (data?.data) {
            ...
            setColors(data.data.map((res: any) => {
                const diffDate: number = differenceInDays(parseISO(res.label), new Date());
                if (diffDate < 0) {
                    return "#fa0019";
                } else if (diffDate === 0) {
                    return "#ebb000";
    ...
    return (
        <>
            ...
            {dueDate && (
                <ModalDetailDueDate isOpen={isOpenModal} dueDate={dueDate} onDidDismiss={() => setIsOpenModal(false)} />
            )}
        </>
    );
}
